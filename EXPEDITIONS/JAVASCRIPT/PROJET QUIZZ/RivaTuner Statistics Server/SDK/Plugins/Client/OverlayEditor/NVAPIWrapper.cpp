...
//////////////////////////////////////////////////////////////////////
BOOL CNVAPIWrapper::GetCurrentTemperatures(DWORD dwGpu, LONG* lpCore, LONG* lpAmbient)
{
	if (dwGpu >= m_dwGpuCount)
		return FALSE;
	...
	{
		for (DWORD dwSensor=0; dwSensor<thermalSettings.count; dwSensor++)
		{
			switch (thermalSettings.sensor[dwSensor].target)
			{
			case NVAPI_THERMAL_TARGET_GPU:
				if (lpCore)
					*lpCore = thermalSettings.sensor[dwSensor].currentTemp;
				break;

			case NVAPI_THERMAL_TARGET_BOARD:
				if (lpAmbient)
					*lpAmbient = thermalSettings.sensor[dwSensor].currentTemp;
				break;
			} 
		}
